...
export default function GeminiAgent() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMsg: ChatMessage = {
        id: `err-${Date.now()}`,
        role: "model",
        text: "My apologies! I failed to sync with the Gemini engine. Please drop a brief line to Grégoire at batchogregoire0@gmail.com!",
        timestamp: new Date().toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              ...
              >
                {/* Avatar */}
                <div
                  className={`w-7 h-7 rounded-full shrink-0 flex items-center justify-center border ${isUser ? "bg-zinc-800 border-zinc-700 text-zinc-300" : "bg-[#d97736]/10 border-[#d97736]/20 text-[#d97736]"}`}
                >
                  {isUser ? (
                ...
                <div>
                  <div
                    className={`px-4 py-3 rounded-2xl text-[12.5px] leading-relaxed shadow-md ${isUser ? "bg-zinc-900 border border-zinc-800 text-zinc-100 rounded-tr-none" : "bg-[#d97736]/5 border border-[#d97736]/10 text-zinc-100 rounded-tl-none"}`}
                  >
      ...
      <form
        onSubmit={handleSend}
        className="p-4 border-t border-zinc-800 bg-neutral-950/60 flex items-center gap-2"
      >
        <input
          type="text"
          value={input}
