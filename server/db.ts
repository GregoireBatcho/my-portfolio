...
class DatabaseStore {
  ...
  private async initMongo() {
    ...
    try {
      ...
      if (remoteProjects.length > 0) {
        this.data.projects = remoteProjects.map(
          ({ _id, ...rest }) => rest,
        ) as any;
      } else {
        await this.mongoDb
      ...
      if (remoteExp.length > 0) {
        this.data.experiences = remoteExp.map(
          ({ _id, ...rest }) => rest,
        ) as any;
      } else {
        await this.mongoDb
      ...
      if (remoteSoft.length > 0) {
        this.data.softSkills = remoteSoft.map(
          ({ _id, ...rest }) => rest,
        ) as any;
      } else {
        await this.mongoDb
  ...
  public async updateSEOSettings(
  ...
  public async addProjectCategory(
    category: Omit<ProjectCategory, "id"> & { id?: string },
  ...
  public async deleteProjectCategory(id: string): Promise<void> {
    await this.ensureConnected();
    this.data.projectCategories = (this.data.projectCategories || []).filter(
      (c) => c.id !== id,
    );
    this.save();
