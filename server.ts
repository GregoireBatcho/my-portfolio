...
if (aiApiKey) {
  try {
    ...
    console.log("Gemini API client initialized successfully in the server context.");
  } catch (error) {
    console.error("Failed to initialize GoogleGenAI client:", error);
  }
} else {
  console.warn("GEMINI_API_KEY is not defined in current environments. AI agent will respond with elegant simulated responses.");
}

// Simple authentication token verification (Session token in-memory store)
const ACTIVE_AUTHENTICATED_SESSIONS = new Set<string>();

function isAdminAuthenticated(req: express.Request, res: express.Response, next: express.NextFunction) {
...
async function startServer() {
  ...
  app.post('/api/auth/login', (req, res) => {
    ...
    if (password === securePassword) {
      ...
      res.json({ token: sessionToken, user: { name: 'Grégoire BATCHO', role: 'admin' } });
    } else {
  ...
  app.post('/api/messages', async (req, res) => {
    try {
      ...
      if (resendKey) {
        try {
          console.log(`[Resend Alert] Attempting real mail dispatch for ${name}...`);
  ...
  app.post('/api/gemini/chat', async (req, res) => {
    ...
    const contextPrompt = `
      ...
      - Contact: ${profile.socials.email || 'batchogregoire81@gmail.com'}
    ...
    if (aiClient) {
      try {
        console.log(`[Gemini Request] Sending context chat query to gemini-3.5-flash...`);
        // Format chat context
        const contents = [
          { role: 'user', parts: [{ text: contextPrompt }] },
          { role: 'model', parts: [{ text: "Understood. I am now set as AI Grégoire BATCHO's assistant ready to help developers and recruiters. I speak English and French." }] }
        ];
        ...
        const reply = geminiRes.text || "I am currently reviewing my notes. Could you please formulate that query again?";
        res.json({ reply });
      } catch (geminiError: any) {
        console.error("Gemini API transaction faulted:", geminiError);
        res.status(500).json({ error: "Gemini transaction faulted. Simulating reply.", reply: `Thanks for asking of my technology work! Currently, my Gemini API connection is under extreme query loads. I would love to talk directly to your engineering team at batchogregoire81@gmail.com!` });
      }
    } else {
      // Simulate highly human-like elegant fallback if no API key is specified
      setTimeout(() => {
        const lower = message.toLowerCase();
        let fallbackReply = `Thanks for reaching out! I'm Grégoire BATCHO. I specialize in building robust full-stack applications with React, TypeScript, and Node.js. Feel free to contact me directly at batchogregoire81@gmail.com for freelance proposals or junior positions!`;
        if (lower.includes('dispo') || lower.includes('avail') || lower.includes('recrut')) {
        ...
        } else if (lower.includes('tech') || lower.includes('stack') || lower.includes('react') || lower.includes('node')) {
